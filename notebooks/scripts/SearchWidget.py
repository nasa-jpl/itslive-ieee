...
class map():
    ...
    def _create_controls(self):
        ...
        slider_dates = [(date.strftime(' %Y-%m-%d '), date) for date in
                        pd.date_range(datetime(1984, 1, 1),
                                      datetime.now(),
                                      freq='D')]
    ...
    def _get_temporal_coverage(self, url):
        ...
        parts = file_name.split('_X_')
        s_date = parts[0].split("_")
        e_date = parts[1].split("_")
        start_date = ""
        end_date = ""
        for item in s_date:
            try:
                if "T" in item:
                    start_date = datetime.strptime(item.split("T")[0], "%Y%m%d").date()
                else:
                    start_date = datetime.strptime(item, "%Y%m%d").date()
                if start_date.year<1990 or start_date.year>2023:
                    continue
                else:
                    break
            except:
        ...
        for item in e_date:
            try:
                if "T" in item:
                    end_date = datetime.strptime(item.split("T")[0], "%Y%m%d").date()
                else:
                    end_date = datetime.strptime(item, "%Y%m%d").date()
                if end_date.year<1990 or end_date.year>2023:
                    continue
                else:
                    break
                break
...
#         print(coverage)
    ...
    def _fetch_urls(self, e):
        ...
        query_params = self.build_query_params()
        url = f'https://staging.nsidc.org/apps/itslive-search/velocities/urls/?{query_params}&serialization=json'
        print(f'querying: {url}')
    ...
    def Search(params: dict):
        ...
        query_url = f'https://staging.nsidc.org/apps/itslive-search/velocities/urls/?{querystring}'
    ...
    def filter_urls(self,
        ...
        """
        # S2B_MSIL1C_20180225T145249_N0206_R053_T14CNB_20180225T205349_X_S2B_MSIL1C_20180307T145249_N0206_R053_T14CNB_20180307T191420_G0120V02_P099.nc
